...
С другой стороны появляется проблема, разработчик на первый взгляд может имть не плохой опыт в пользовании конкретным фреймворком, но плохо понимать язык, который лежит в основе этого фреймворка и, как следствие, допускать допускать ошибки, которые приводят к проблемам в продакшене.
...
На практике это отлично работает для стандартных случаев (именно для них и пишутся фреймворки), для простых и типовых приложинеий, у которых нет особой нагрузки. 
...
В этот момент в ход идут костыли, которые рабоют по принципу "решаем одну проблему - добавляем две новых".
