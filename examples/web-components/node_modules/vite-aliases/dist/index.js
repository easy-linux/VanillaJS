...
// src/utils.ts
init_cjs_shims();
var import_consola = __toModule(require("consola"));
...
var logger = import_consola.default.create({});
...
function getDirectories(gen) {
  const { dir, root, deep, depth } = gen.options;
  const directories = import_fast_glob.default.sync(deep ? `${dir}/**/*` : `${dir}/*`, {
    ignore: ["node_modules"],
...
function writeConfig(gen) {
  ...
  if ((0, import_fs.existsSync)(file)) {
    const data = (0, import_fs.readFileSync)(`${file}`).toString();
    let errors = [];
    json = (0, import_jsonc_parser.parse)(data, errors, { disallowComments: true });
    if (!empty(errors)) {
      throw logger.error(new Error(`Can't read JSON Config File, if it contains Comments please remove them.For more Information: https://github.com/Subwaytime/vite-aliases/issues/25`));
    }
    if (json.compilerOptions) {
      const paths = json.compilerOptions.paths || {};
      json.compilerOptions.paths = __spreadValues(__spreadValues({}, paths), gen.paths);
    } else {
      json.compilerOptions = {
        paths: __spreadValues({}, gen.paths)
      };
    }
  } else {
    IDEConfig.compilerOptions.paths = __spreadValues({}, gen.paths);
    json = Object.assign({}, IDEConfig);
  }
  (0, import_fs.writeFile)(`${file}`, JSON.stringify(json, null, 4), (error) => {
    if (error) {
...
function writeLog(gen) {
  ...
  (0, import_fs2.mkdir)(`${folder}`, { recursive: true }, (error) => {
    (0, import_fs2.writeFile)(`${file}`, JSON.stringify(data, null, 4), (error2) => {
      if (error2) {
...
var Generator = class {
  constructor(servermode, options) {
    ...
    this.fullPath = slash((0, import_path.resolve)(root, dir));
    if (servermode === "serve") {
      const watcher = import_chokidar.default.watch(this.fullPath, { ignoreInitial: true, depth });
